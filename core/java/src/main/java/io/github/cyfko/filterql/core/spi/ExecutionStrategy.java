 ...
 * <ul>
 * <li><b>HOW</b> to execute the query: single query, multi-query, native SQL,
 * cached, etc.</li>
 * <li><b>WHAT</b> to return: List, Page, Stream, Count, custom wrapper,
 * etc.</li>
 * </ul>
 * </p>
 *
 * <p>
 * This dual responsibility allows for maximum flexibility: new execution
 * approaches
 * and new return types can be added without modifying the {@link QueryExecutor}
 * interface.
 * </p>
 *
 ...
 * Executes a single query with joins, returns a List</li>
 * <li><b>MultiQueryExecutionStrategy&lt;List&lt;R&gt;&gt;</b>:
 * Splits into multiple optimized subqueries for collections, returns a
 * List</li>
 ...
 *         String cacheKey = generateKey(resolver, params);
 *         return cache.get(cacheKey, () -> {
 *             // Fallback to actual query execution
 *             return executeActualQuery(ctx, resolver, params);
 *         });
 *     }
 *
 *     {@literal @}Override
 ...
 * If state is required (e.g., caching, configuration), ensure proper
 * synchronization.
 * </p>
 *
...
public interface ExecutionStrategy<R> {
     ...
     * <li>Applying filters from the ConditionResolver</li>
     ...
     */
    <Context> R execute(Context ctx, ConditionResolver<Context, ?> resolver, QueryExecutionParams params);
}