...
/**
 * Framework-agnostic interface for bridging filter definitions to
 * backend-specific query execution.
 * <p>
 * {@code FilterContext} serves as the central abstraction that transforms
 * high-level filter
 * definitions (property references, operators, filter keys) into executable
 * {@link Condition}
 * and {@link ConditionResolver} instances. It decouples the core filtering
 * logic from
 ...
 * <ul>
 * <li><strong>Condition Generation:</strong> Transform filter metadata into
 * composable {@link Condition} instances</li>
 * <li><strong>Predicate Resolution:</strong> Convert logical conditions into
 * backend-specific query predicates</li>
 * <li><strong>Deferred Value Binding:</strong> Support structural caching by
 * separating filter structure from values</li>
 * <li><strong>Type Validation:</strong> Ensure type compatibility between
 * properties, operators, and values</li>
 * </ul>
 *
 * <h2>Deferred Argument Architecture</h2>
 * <p>
 * This interface supports a <strong>deferred argument pattern</strong> where
 * filter values are
 * separated from filter structure:
 * </p>
 * <ol>
 * <li><strong>Phase 1 ({@link #toCondition}):</strong> Create {@link Condition}
 * structure WITHOUT values
 * using only property references, operators, and argument keys</li>
 * <li><strong>Phase 2 ({@link #toResolver}):</strong> Bind actual values via
 * argument registry
 * and convert to executable {@link ConditionResolver}</li>
 * </ol>
 * <p>
 * This separation enables <strong>structural caching</strong>: conditions with
 * the same structure
 ...
 * <h2>Thread Safety</h2>
 * <p>
 * Implementations must be thread-safe for concurrent filter resolution.
 * Condition creation
 ...
 * <ul>
 * <li>Validate property references and operator compatibility in
 * {@link #toCondition}</li>
 * <li>Make NO assumptions about the semantics of filter keys (they are opaque
 * identifiers)</li>
 ...
 * <li>Defer actual query predicate construction until {@link #toResolver} is
 * called</li>
 * </ul>
 *
 ...
 * Each adapter translates {@link Condition} instances into backend-specific
 * predicates.
 * </p>
 *
 ...
 * @since 4.0
 */
public interface FilterContext<Ex> {

    /**
     * Transforms the input {@link FilterDefinition} into a {@link Condition} that
     * can be subsequently composed.
     * <p>
     * This method transforms a filter definition consisting of a property reference
     * and an operator
     * into a concrete condition that can be composed with other {@link Condition}s.
     * The filter argument value is <strong>not</strong> provided here—it will be
     * retrieved later
     * from the argument registry when
     * {@link #toResolver(Condition, QueryExecutionParams)} is called.
     * This deferred approach enables structural caching where conditions with the
     * same structure
     ...
     * // The value will be retrieved later from the argument registry using
     * // "emailArg" key
     ...
     * }</pre>
     *
     * @param argKey the key used to retrieve the filter argument's value from the
     *               argument registry
     *               when {@link #toResolver(Condition, QueryExecutionParams)} is
     *               invoked
     * @param ref    the reference to the property of interest
     * @param op     the operator applied to the property (case-insensitive).
     *               Can be standard operators ("EQ", "NE", "GT", "LT", "GE", "LE",
     *               "LIKE", "IN", etc.)
     ...
     * @throws FilterDefinitionException if the filter definition cannot be
     *                                   translated to a {@link Condition}
     * @throws NullPointerException      if any argument is null
     */
    <P extends Enum<P> & PropertyReference> Condition toCondition(String argKey, P ref, String op)
            throws FilterDefinitionException;

    /**
     * Converts a logical {@link Condition} tree into an executable
     ...
     * </p>
     * <ol>
     * <li>Binding argument keys to their concrete values via
     * {@link QueryExecutionParams#arguments()}</li>
     * <li>Applying optional projection via
     * {@link QueryExecutionParams#projection()}</li>
     * <li>Transforming the abstract condition tree into a backend-specific
     * predicate resolver</li>
     * <li>Returning an executor that can generate query predicates on-demand</li>
     * </ol>
     * <p>
     * The returned {@link ConditionResolver} encapsulates backend-specific query
     * generation logic
     ...
     * </p>
     * <ul>
     * <li><strong>arguments:</strong> Map of argument keys (from
     * {@link #toCondition}) to values</li>
     * <li><strong>projection:</strong> Optional set of DTO field paths (null = full
     * entity)</li>
     * </ul>
     * <p>
     ...
     * @param params    runtime execution parameters (arguments + optional
     *                  projection)
     * @return a {@link ConditionResolver} capable of generating backend-specific
     *         predicates
     * @throws IllegalArgumentException      if {@code condition} is incompatible
     *                                       with this context
     * @throws UnsupportedOperationException if the condition cannot be converted to
     *                                       the target backend
     * @throws NullPointerException          if any argument is null
     * @throws FilterDefinitionException     if transformation fails due to invalid
     *                                       filter definitions
     * @throws IllegalStateException         if an argument key in the condition is
     *                                       missing from the parameter registry,
     *                                       or if projection is requested but not
