 ...
 * context management, and predicate resolution into a single, easy-to-use
 * interface.
 * It orchestrates the entire filtering pipeline from filter request to
 ...
 * </p>
 * <ol>
 * <li><strong>Parse:</strong> Transform DSL expression into FilterTree using
 * {@link DslParser}</li>
 * <li><strong>Populate:</strong> Register filter definitions in
 * {@link FilterContext}</li>
 * <li><strong>Generate:</strong> Create condition tree from FilterTree and
 * Context</li>
 * <li><strong>Resolve:</strong> Convert conditions into executable
 * {@link ConditionResolver}</li>
 * </ol>
 *
 * <p>
 ...
 * );
 *
 * // 4. Resolve to executable condition resolver
 ...
 * <li>{@link IllegalArgumentException} - Invalid filter definitions or
 * mappings</li>
 * </ul>
 *
 * <p>
 ...
 * </p>
 * <ul>
 * <li>Reuse FilterQueryFactory instances when possible (they are
 * thread-safe)</li>
...
public class FilterQueryFactory {
    ...
    /**
     * Creates a {@link FilterQuery} with the default {@link BasicDslParser} and the
     * given context.
     * <p>
     * This is the most commonly used factory method. It provides the standard DSL
     * parsing
     * capabilities while allowing customization of the context and mapping
     * strategies.
     * </p>
     *
     ...
     */
    public static <Em> FilterQuery<Em> of(FilterContext<Em> context) {
     ...
     * @return A new FilterQueryFactory instance configured with the provided
     *         components
     * @throws NullPointerException if dslParser or context is null
     */
    public static <Em> FilterQuery<Em> of(FilterContext<Em> context, DslParser dslParser) {
        return new DefaultFilterQuery<>(context, dslParser, ProjectionPolicy.defaults());
    }

    /**
     * Creates a {@link FilterQuery} with the given parser, context, and projection
     * policy.
     *
     * @param context   The context to use for condition resolution. Must not be
     *                  null.
     ...
     */
    public static <Em> FilterQuery<Em> of(FilterContext<Em> context, DslParser dslParser, ProjectionPolicy policy) {
        return new DefaultFilterQuery<>(context, dslParser, policy);
    }

    /**
     * Creates a {@link FilterQuery} with the default {@link BasicDslParser}, the
     * given context, and projection policy.
     *
     * @param context The context to use for condition resolution. Must not be null.
     ...
     * @throws NullPointerException if any parameter is null.
     */
    public static <Em> FilterQuery<Em> of(FilterContext<Em> context, ProjectionPolicy policy) {
     ...
     * <p>
     * This record encapsulates the context, DSL parser, and projection policy
     * required
    ...
    private record DefaultFilterQuery<Em>(FilterContext<Em> filterContext,
        ...
        @Override
        public <P extends Enum<P> & PropertyReference> ConditionResolver<Em, ?> toResolver(FilterRequest<P> request) {
            ...
            // Convert condition into a ConditionResolver with deferred filter arguments
            ...
            return filterContext.toResolver(condition, params);
        }

        @SuppressWarnings({ "unchecked", "rawtypes" })
        @Override
        public <P extends Enum<P> & PropertyReference> QueryExecutor<Em> toExecutor(FilterRequest<P> request) {
            ConditionResolver<Em, ?> cr = toResolver(request);
         ...
         * <p>
         * This method extracts the values from each {@link FilterDefinition} in the
         * input map
         * and collects them into a new map keyed by the same keys, effectively forming
         * a registry of current filter parameter values for use during query
         * processing.
         * </p>
         *
         * @param definitionMap the map of filter keys to their corresponding
         *                      {@link FilterDefinition} objects
         * @return a new map mapping each filter key to the current value from its
         *         filter definition
         * @throws NullPointerException if {@code definitionMap} is null or contains
         *                              null keys or values
         */
     ...
     * <ul>
     * <li>a {@link ConditionResolver} (resolving the condition)</li>
    ...
    private record DefaultQueryExecutor<Em>(ConditionResolver<Em, ?> resolver,
         ...
         */
        @Override
        public <R> R executeWith(Em em, ExecutionStrategy<R> strategy) {
            Objects.requireNonNull(em, "Execution management context cannot be null");
            ...
            log.info(() -> String.format("FilterQL query executed successfully in %d ms", durationMs));
