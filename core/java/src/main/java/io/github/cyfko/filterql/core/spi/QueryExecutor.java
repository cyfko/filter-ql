 ...
 * appropriately
 * for thread safety (e.g., per-thread PersistenceContext).
 * </p>
 *
...
public interface QueryExecutor<Em> {
     ...
     * <ul>
     * <li>The specific implementation defined execution management context (e.g.,
     ...
     * List<?> users = executor.executeWith(context, new MultiQueryFetchStrategy<>());
     *
     * // Execute to get a page of results
     * Page<UserDto> page = executor.executeWith(context, new PagedFetchStrategy<>());
     *
     * // Execute to get a count
     * Long count = executor.executeWith(context, new CountStrategy<>());
     * }</pre>
     *
