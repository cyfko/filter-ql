 ...
 *
 * <p>
 * Facade interface representing the central entry point for the entire
 * lifecycle
 ...
 * the complexity of query building and execution, offering multiple levels of
 * control:
 * </p>
 *
 * <ul>
 * <li><b>Low-level:</b> Obtain a {@link ConditionResolver} for manual query
 * building</li>
 * <li><b>Mid-level:</b> Obtain a {@link QueryExecutor} and choose an
 * {@link ExecutionStrategy}</li>
 ...
 *          ▼                              ▼
 * ┌──────────────────┐         ┌──────────────────┐
 * │ConditionResolver │         │  QueryExecutor   │
 * └──────────────────┘         └────────┬─────────┘
 *                                       │
 *                                       ▼
 *                              ┌──────────────────┐
 ...
 * }</pre>
 *
 * <h4>Example 3: Low-level with condition resolver (maximum control)</h4>
 * 
 * <pre>{@code
 * FilterRequest<UserProperty> request = ...;
 *
 ...
 * <ul>
 * <li><b>Flexibility:</b> Works with entities, DTOs, Tuples, Maps, or custom
 * types</li>
 * <li><b>Extensibility:</b> New execution strategies can be added without
 * changing this interface</li>
 * <li><b>Testability:</b> Each component (resolver, executor, strategy) can be
 * tested independently</li>
 * <li><b>Separation of Concerns:</b> Filter building, query execution, and
 * result transformation are separated</li>
 * </ul>
 *
 * <h3>Thread Safety:</h3>
 * <p>
 * Implementations of {@code FilterQuery} should be thread-safe and reusable
 * across multiple
...
public interface FilterQuery<Em> {
    ...
    /**
     * Constructs a {@link ConditionResolver} capable of translating a
     * {@link FilterRequest}
     * into backend-specific predicates.
     *
     * <p>
     ...
     * @return a {@link ConditionResolver} ready to generate backend-specific
     *         predicates
     * @throws IllegalArgumentException if request is null or invalid
     */
    <P extends Enum<P> & PropertyReference> ConditionResolver<Em, ?> toResolver(FilterRequest<P> request);
     ...
     *
     * <p>
     * Use this method when you want to execute the same query with different
     * strategies,
     ...
     * }</pre>
     *
     * @param request FilterQL request describing projection, filters, sorting, and
     *                pagination
     ...
     */
    <P extends Enum<P> & PropertyReference> QueryExecutor<Em> toExecutor(FilterRequest<P> request);

    /**
     * Executes a FilterQL request in a single method call using the specified
     * strategy.
     *
     * <p>
     * This is the highest-level convenience method that handles the entire query
     * lifecycle:
     * resolving the executor, executing the query, and returning results.
     * </p>
     *
     * <p>
     * The return type {@code T} is inferred from the strategy's generic type
     * parameter,
