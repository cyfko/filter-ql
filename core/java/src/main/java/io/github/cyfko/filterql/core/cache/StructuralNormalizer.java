 ...
 * while ignoring filter names and values.
 * <p>
 * This normalizer provides the highest cache hit rate by treating filters with the same
 ...
 * @since 4.0.0
 */
public class StructuralNormalizer implements DslNormalizer {
     ...
     *
     * @param definitionMap map of filter definitions.
     * @param identifiers the list of identifiers to replace
     * @return normalized and simplified structure string (compact, no spaces)
     */
    public static <P extends Enum<P> & PropertyReference> String normalizeStructure(Map<String, FilterDefinition<P>> definitionMap, List<String> identifiers) {
        StringBuilder normalized = new StringBuilder();

        for (String key : identifiers) {
            FilterDefinition<?> definition = definitionMap.get(key);
            if  (definition != null) {
