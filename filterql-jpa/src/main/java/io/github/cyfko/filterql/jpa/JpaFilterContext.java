...
/**
 * JPA Criteria API implementation of {@link FilterContext} that materializes
 * FilterQL
 ...
 * <li>Type conversion rules encapsulated in {@link FilterConfig} and
 * {@link TypeConversionUtils}</li>
 * <li>Custom predicate implementations via
 * {@link PredicateResolverMapping}</li>
 * </ul>
 *
 * <p>
 ...
 * </p>
 * <ol>
 * <li>{@link #toCondition(String, Enum, String)} builds a {@link Condition}
 * bound to a property and operator but not yet to concrete values.</li>
 * <li>{@link #toResolver(Condition, QueryExecutionParams)} injects argument
 * values and returns a {@link PredicateResolver} for Criteria queries.</li>
 * </ol>
 *
 ...
 */
public class JpaFilterContext<P extends Enum<P> & PropertyReference> implements FilterContext<EntityManager> {

    /**
     * Global filter behavior configuration (null handling, enum matching, string
     * case, etc.).
     */
     ...
     *
     * <p>
     * Suppliers created in {@link #toCondition(String, Enum, String)} resolve their
     * actual
     * argument values from this registry after
     * {@link #toResolver(Condition, QueryExecutionParams)}
     ...
     * <p>
     * When this resolver returns a non-null {@link PredicateResolver}, it is
     * used
     ...
     *
     * <p>
     * Uses {@link FilterConfig#builder()} defaults for null handling, enum
     * matching,
     * and string normalization. The mapping function is responsible for mapping
     * each
     * property enum to either a JPA path or a
     * {@link PredicateResolverMapping}.
     * </p>
     *
     * @param enumClass      Property enum class (must implement
     *                       {@link PropertyReference})
     * @param mappingBuilder Mapping function returning path or
     *                       {@link PredicateResolverMapping}
     ...
     * <p>
     * This constructor initializes the context with the required type information
     * and
     ...
     *
     * <p>
     * <strong>Mapping Function Requirements:</strong>
     * </p>
     * <p>
     * The mapping function must return one of the following types:
     * </p>
     * <ul>
     ...
     *
     * <p>
     * <strong>Simple Property Path Examples:</strong>
     * </p>
     * 
     * <pre>{@code
     ...
     *
     * <p>
     * <strong>Custom Mapping Examples:</strong>
     * </p>
     * 
     * <pre>
     * {
     ...
     *
     * <p>
     * Can be used to switch mapping strategies at runtime (for example, based on
     * tenant,
     * profile, or feature flags). Existing {@link Condition} instances retain the
     * mapping
     * they were built with.
     * </p>
     *
     * <p>
     * <strong>Thread-safety:</strong> Not thread-safe. If called concurrently with
     * query execution, external synchronization is required.
     * </p>
     *
     ...
     * <p>
     * The resolver is called <strong>before</strong> the default resolution
     * mechanism
     * (path-based or {@link PredicateResolverMapping}). If the resolver
     * returns a
     * non-null {@link PredicateResolver}, it is used directly. If it returns
     * {@code null},
     * the default handling is applied.
     * </p>
     *
     * <p>
     * <strong>Example - Custom SOUNDEX operator:</strong>
     * </p>
     * 
     * <pre>{@code
     * JpaFilterContext<UserProperty> context = new JpaFilterContext<>(
     *         UserProperty.class, mappingBuilder).withCustomOperatorResolver((ref, op, args) -> {
    ...
    /**
     * Creates a {@link Condition} for the given argument key, property reference,
     * and operator.
     *
     * <p>
     * Validates argument key, reference type compatibility, and operator support.
     * The created
     * {@link Condition} holds a {@link PredicateResolver} that retrieves its
     * parameter
     * lazily from the thread-local arguments registry.
     * </p>
     * 
     * <p>
     * For custom operators (Op.CUSTOM), the property's mapping function must return
     * a
     * {@link PredicateResolverMapping} instead of a path string.
     * </p>
     *
     * @param argKey Argument key used to look up values in
     *               {@link QueryExecutionParams#arguments()}
     ...
     * @return Condition bound to property and operator, but not yet to concrete
     *         values
     *
     * @throws IllegalArgumentException if key is blank, ref is null, or type
     *                                  mismatches
     */
    @Override
     ...
     *
     * <p>
     * Collections and arrays are expanded into a new array; single values are
     * wrapped.
     * </p>
     *
    ...
    /**
     * Converts a {@link Condition} into a {@link PredicateResolver} bound to the
     * given parameters.
     *
     * <p>
     * Loads {@link QueryExecutionParams#arguments()} into the thread-local registry
     * and
     * extracts the underlying {@link PredicateResolver} from the condition
     * (expected to be
     * a {@link JpaCondition}).
     * </p>
     *
    ...
    /**
     * Extracts the underlying {@link PredicateResolver} from a
     * {@link Condition}.
     *
     * @param condition Condition instance (must be {@link JpaCondition})
     * @return Resolver extracted from the condition
     * @throws IllegalStateException if the condition type is not
     *                               {@link JpaCondition}
     */
    ...
    /**
     * Builds a {@link PredicateResolver} from a JPA path and a deferred
     ...
     * @param definitionSupplier Supplier providing a {@link FilterDefinition} with
     *                           raw value
     * @param <E>                Entity type
        ...
        return (Root<E> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {
            ...
            return switch (definition.operator()) {
                ...
                case GTE -> buildOrPredicateIfCollection(
                        castToComparablePath(path),
                        convertedValue,
                        (Expression<Comparable> p, Object v) -> cb.greaterThanOrEqualTo(p,
                                (Comparable) castToComparable(v)),
                        cb);
                ...
                case LTE -> buildOrPredicateIfCollection(
                        castToComparablePath(path),
                        convertedValue,
                        (Expression<Comparable> p, Object v) -> cb.lessThanOrEqualTo(p,
                                (Comparable) castToComparable(v)),
                        cb);
     ...
     * {@link TypeConversionUtils} according to {@link FilterConfig} enum match
     * mode.
     * </p>
     *
    ...
    /**
     * Builds an OR-composed predicate when value is a collection; otherwise a
     * single predicate.
     *
    ...
    /**
     * Builds an AND-composed predicate when value is a collection; otherwise a
     * single predicate.
     *
     * @param expression       JPA expression for the path
     * @param value            Single value or collection
    ...
    /**
     * Casts a generic {@link Path} to a {@link Comparable} path for comparison
     * operations.
     *
...
// import io.github.cyfko.filterql.core.spi.JpaPredicateResolver;
// import io.github.cyfko.filterql.core.utils.TypeConversionUtils;
// import io.github.cyfko.filterql.core.utils.FilterConfigUtils;
// import io.github.cyfko.filterql.core.validation.Op;
// import io.github.cyfko.filterql.core.validation.PropertyReference;
// import io.github.cyfko.filterql.jpa.mappings.JpaPredicateResolverMapping;
// import io.github.cyfko.filterql.jpa.utils.PathResolverUtils;
// import jakarta.persistence.criteria.*;
//
// import java.lang.reflect.Array;
...
//
/// **
// * JPA Criteria API implementation of {@link FilterContext} for dynamic query
// filtering.
// * <p>
// * This class provides a type-safe bridge between FilterQL filter definitions
// and JPA
// * Criteria API predicates. It handles the complete lifecycle of filter
// transformation:
// * from property references to database predicates, including type conversion,
// validation,
...
// * <ul>
// * <li><strong>Type Safety:</strong> Enum-based property references to prevent
// typos and enable IDE support</li>
// * <li><strong>Automatic Type Conversion:</strong> Handles all JPA types via
// {@link TypeConversionUtils}:
// * primitives, dates, enums, UUIDs, collections, and custom types</li>
// * <li><strong>Flexible Mapping:</strong> Supports simple property paths and
// custom resolver logic</li>
// * <li><strong>Collection Support:</strong> Automatic OR/AND logic for
// multi-value filters</li>
// * <li><strong>Nested Properties:</strong> Dot notation for deep object graphs
// (e.g., "address.city.name")</li>
// * <li><strong>Custom Operators:</strong> Extensible operator system via
// {@link CustomOperatorProvider}</li>
// * <li><strong>Configurable Behavior:</strong> Fine-grained control through
// {@link FilterConfig}</li>
// * </ul>
// *
// * <h2>Architecture</h2>
// * <p>
...
// * <li>{@link #toCondition(String, Enum, String)} validates and maps to
// conditions</li>
// * <li>Mapping function returns either a path string or custom resolver</li>
// * <li>{@link #toCondition(String, Enum, String)} assembles conditions into a
// final JPA predicate</li>
...
// * </dd>
// *
// * <dt><strong>JpaPredicateResolverMapping</strong></dt>
// * <dd>
// * For custom business logic requiring manual predicate construction.
// * Useful for computed properties, multi-field searches, or complex
// conditions.
// * <pre>{@code
// * case AGE_RANGE -> new JpaPredicateResolverMapping<User>() {
// * @Override
// * public Predicate resolve(Root<E> root, CriteriaQuery<?> q, CriteriaBuilder
// cb, Object[] params) {
...
// * <p>
// * Type conversion is handled automatically by {@link TypeConversionUtils} and
// supports:
// * </p>
// * <ul>
// * <li>Primitives and wrappers (int, Integer, long, Long, etc.)</li>
// * <li>Numeric types (BigDecimal, BigInteger, Double, Float, etc.)</li>
// * <li>Date/Time types (LocalDate, LocalDateTime, Instant, ZonedDateTime,
// etc.)</li>
...
// * <ul>
// * <li><strong>NullValuePolicy:</strong> REJECT (throw error) or IGNORE (skip
// filter)</li>
// * <li><strong>StringCaseStrategy:</strong> NONE, UPPER, LOWER for string
// normalization</li>
...
// * <h2>Thread Safety</h2>
// * <p>
// * <strong>Instance fields are immutable after construction.</strong> However,
// the class
// * uses a {@link ThreadLocal} registry for argument values during predicate
// resolution.
// * This makes individual instances thread-safe for concurrent filter
// execution, but
...
// * <li><strong>Condition Creation:</strong> Validates operator support and
// enum type compatibility</li>
// * <li><strong>Resolution:</strong> Validates argument presence and type
// compatibility</li>
// * </ul>
// * <p>
...
// * @see FilterConfig
// * @see TypeConversionUtils
// * @see JpaPredicateResolverMapping
// */
...
// * This ensures thread-safety when JpaFilterContext instances are shared
// across multiple requests.
// * The argumentsRegistry is populated in toResolver() and accessed by
// Suppliers created in toCondition().
// */
// private final ThreadLocal<Map<String, Object>> argumentsRegistry =
// ThreadLocal.withInitial(HashMap::new);
//
// private Function<P, Object> mappingBuilder;
//
// /**
// * Returns the enum class used as a property reference in filtering or
// querying.
// * <p>
// * This method provides the {@code Class} object representing the enum type
// {@code P}
// * that defines the property references in the filtering framework. It
// facilitates
// * generic handling of filters, dynamic criteria adjustments, and
// interpretation of
// * enumerated values used in data operations.
// * </p>
// *
// * @return the {@code Class} instance corresponding to the enum type {@code
// P}.
// */
// public Class<P> getPropertyRefClass() {
// return enumClass;
...
//
// /**
// * Constructs a new FilterContext for the specified entity and property
// reference types without projection support.
// * <p>
// * This constructor initializes the context with the required type information
// and
// * mapping strategy. The mapping function determines how property references
// are
// * translated into concrete filter implementations. This constructor does NOT
// support projection.
// * </p>
// *
...
// * <pre>{@code
// * Function<FilterDefinition<UserPropertyRef>, Object> simpleMapping = def ->
// switch (def.ref()) {
// * case NAME -> "name"; // Direct property access
// * case EMAIL -> "email"; // Direct property access
// * case AGE -> "age"; // Direct property access
...
// * <pre>{@code
// * Function<FilterDefinition<UserPropertyRef>, Object> complexMapping = def ->
// switch (def.ref()) {
// * // Simple path mappings
// * case NAME -> "name";
// * case EMAIL -> "email";
...
// * @Override
// * public Predicate resolve(Root<E> root, CriteriaQuery<?> query,
// CriteriaBuilder cb, Object[] params) {
// * String searchTerm = (String) params[0];
// * return cb.or(
// * cb.like(cb.lower(root.get("firstName")), "%" + searchTerm.toLowerCase() +
// "%"),
...
// * validate the mapping function's behavior. Invalid mappings will be
// detected.
// * </p>
// *
// * @param enumClass The class of the property reference enum (e.g.,
// UserPropertyRef.class)
// * @param mappingBuilder The function that maps filter definitions to filter
// implementations.
// * Must return either a String (property path) or
// JpaPredicateResolverMapping
// * @throws NullPointerException if any parameter is null
// * @see JpaPredicateResolverMapping
// * @see PropertyReference
// */
// public JpaFilterContext(Class<P> enumClass, Function<P, Object>
// mappingBuilder) {
// this(enumClass, mappingBuilder, FilterConfig.builder().build());
// }
//
...
// * BiFunction<UserProperty, String, Object> mapping = (prop, op) -> switch
// (prop) {
// * case NAME -> "name";
// * case EMAIL -> "email";
// * case CITY -> "address.city.name";
// * case FULL_NAME_SEARCH -> (JpaPredicateResolverMapping<User>)
...
// * @param mappingBuilder Function mapping property references to paths or
// resolvers (must not be null)
// * @param filterConfig Configuration object for filtering strategies (must not
// be null)
// * @throws NullPointerException if enumClass, mappingBuilder, or filterConfig
// is null
// * @since 2.0.0
// * @see FilterConfig
// * @see JpaPredicateResolverMapping
...
// FilterConfig filterConfig
// ) {
// this.enumClass = Objects.requireNonNull(enumClass, "enumClass cannot be
// null");
// this.mappingBuilder = Objects.requireNonNull(mappingBuilder, "mappingBuilder
// cannot be null");
// this.filterConfig = Objects.requireNonNull(filterConfig, "filterConfig cannot
// be null");
// }
//
// /**
...
// * <p>
// * This method allows dynamic reconfiguration of how property references are
// mapped
// * to filter implementations. It's useful for scenarios where mapping logic
// needs
// * to be updated at runtime, such as multi-tenant applications or A/B testing
// * of different filtering strategies.
// * </p>
// *
// * <p><strong>Thread Safety:</strong> This method is not thread-safe. If
// called
...
// * BiFunction<UserProperty, String, Object> adminMapping = (prop, op) ->
// switch (prop) {
// * case NAME -> "name";
// * case EMAIL -> "email";
...
// * };
// *
// * BiFunction<UserProperty, String, Object> oldMapping =
...
// * <ul>
// * <li>Affects all subsequent calls to {@link #toCondition(String, Enum,
// String)}</li>
// * <li>Previously created {@link Condition} objects use their original
// mappings</li>
// * <li>The previous builder is returned to enable restoration or fallback</li>
// * <li>Must not be null (enforced via {@link
// Objects#requireNonNull(Object)})</li>
// * </ul>
// *
// * @param mappingBuilder the new function to transform property references to
// paths or resolvers, must not be null
// * @return the previous mapping builder that was set, never null
// * @throws NullPointerException if mappingBuilder is null
// */
// public Function<P, Object> setMappingBuilder(Function<P, Object>
// mappingBuilder) {
// var prev = this.mappingBuilder;
// this.mappingBuilder = Objects.requireNonNull(mappingBuilder);
// return prev;
...
//
// @Override
// public <Q extends Enum<Q> & PropertyReference> Condition toCondition(String
// argKey, Q ref, String op) {
// // Ensure constraints are met
// if (argKey == null || argKey.trim().isEmpty()) {
// throw new IllegalArgumentException("Filter argument key cannot be null or
// empty");
// }
// if (ref == null) {
// throw new IllegalArgumentException("Filter definition cannot be null");
...
// ));
// }
// if (Op.fromString(op) == Op.CUSTOM &&
// OperatorProviderRegistry.getProvider(op).isEmpty()) {
...
//
// // Create Supplier that retrieves value from thread-local registry when
// invoked
...
// throw new IllegalStateException(
// "Argument key '" + argKey + "' not found in argumentsRegistry. " +
// "Ensure toResolver() is called with a registry containing all required
// argument keys."
// );
// }
// return registry.get(argKey);
// };
//
// if (mapping instanceof JpaPredicateResolverMapping<?> prm) {
// @SuppressWarnings({"rawtypes", "unchecked"})
// JpaPredicateResolver<?> resolver = (root, query, cb) -> {
// Object[] paramArray = toParameterArray(param.get());
...
//
// if (mapping instanceof String pathName) {
// JpaPredicateResolver<?> resolver = getResolverFromPath(
// this.filterConfig,
// pathName,
// () -> new FilterDefinition<>(ref, op, param.get())
// );
...
// }
//
// throw new IllegalArgumentException("Invalid mapping function unsupported
// yet");
// }
//
// private static Object[] toParameterArray(Object p) {
...
//
// @Override
// public JpaPredicateResolver<?> toResolver(Condition condition,
// QueryExecutionParams params) {
// // Validate inputs
// if (condition == null) {
// throw new IllegalArgumentException("condition cannot be null");
...
// // This ensures that the Suppliers created in toCondition() will retrieve the
// correct values
...
// return extractJpaPredicateResolver(condition);
// }
//
// /**
// * Extracts JpaPredicateResolver from Condition.
// */
// private JpaPredicateResolver<?> extractJpaPredicateResolver(Condition
// condition) {
// try {
// JpaCondition<?> jpaCondition = (JpaCondition<?>) condition;
// return jpaCondition.getResolver();
...
//
// /**
// * Builds a JPA JpaPredicateResolver from a property path and filter
// definition.
// * <p>
// * This method is the core conversion engine that transforms FilterQL
// definitions into
// * JPA Criteria API predicates. It handles type conversion, collection
// support, and
...
// * <ul>
// * <li><strong>Automatic Type Conversion:</strong> Converts values to match
// the target property type</li>
// * <li><strong>Collection Support:</strong> Singleton operators (EQ, NE, GT,
// etc.) support collections with OR/AND logic</li>
// * <li><strong>Nested Path Resolution:</strong> Handles complex paths like
// "address.city.name"</li>
...
// * <td>LOWER(field) NOT LIKE LOWER(p1) AND LOWER(field) NOT LIKE
// LOWER(p2)</td>
// * </tr>
// * <tr>
// * <td>IN</td>
...
// *
// * // String to LocalDate
// * FilterDefinition(BIRTH_DATE, GT, "2000-01-01") → WHERE birthDate >
// '2000-01-01'
// *
...
// *
// * // Long to Date
// * FilterDefinition(CREATED_AT, LT, 1640000000000L) → WHERE createdAt <
// '2021-12-20...'
// *
...
// * <li>Dates: LocalDate, LocalDateTime, LocalTime, Instant, ZonedDateTime,
// java.util.Date</li>
// * <li>UUID</li>
...
// * @throws IllegalArgumentException if type conversion fails or path is
// invalid
// */
// @SuppressWarnings({"unchecked", "rawtypes"})
// public static <E, P extends Enum<P> & PropertyReference>
// JpaPredicateResolver<E>
// getResolverFromPath(
// FilterConfig filterConfig,
// String pathName,
// Supplier<FilterDefinition<P>> definitionSupplier) {
...
// definition = FilterConfigUtils.ensureNullValuePolicy(filterConfig,
// definition);
//
...
// Object convertedValue = convertValueForPath(definition, pathJavaType,
// filterConfig);
//
// // Switch on supported operators to construct a predicate
// return switch (definition.operator()) {
// case EQ -> buildOrPredicateIfCollection(path, convertedValue, cb::equal, cb);
// case NE -> buildAndPredicateIfCollection(path, convertedValue, cb::notEqual,
// cb);
// case GT -> buildOrPredicateIfCollection(
// castToComparablePath(path),
// convertedValue,
// (Expression<Comparable> p, Object v) -> cb.greaterThan(p, (Comparable)
// castToComparable(v)),
// cb
// );
// case GTE -> buildOrPredicateIfCollection(
...
// castToComparablePath(path),
// convertedValue,
// (Expression<Comparable> p, Object v) -> cb.lessThan(p, (Comparable)
// castToComparable(v)),
// cb
// );
// case LTE -> buildOrPredicateIfCollection(
// castToComparablePath(path),
// convertedValue,
// (Expression<Comparable> p, Object v) -> cb.lessThanOrEqualTo(p, (Comparable)
// castToComparable(v)),
// cb
// );
//
...
// TypeConversionUtils.applyStringCaseStrategy(convertedValue,
// filterConfig.getStringCaseStrategy());
// Expression<String> pathExpr = switch (filterConfig.getStringCaseStrategy()) {
// case LOWER -> cb.lower(expPath);
// case UPPER -> cb.upper(expPath);
// default -> expPath;
...
// Object[] valuesToCompare = collection.toArray();
// if (valuesToCompare.length != 2) {
// throw new IllegalArgumentException("RANGE operator requires exactly 2
// values");
// }
// yield buildBetweenPredicate(path, valuesToCompare[0], valuesToCompare[1],
// cb);
// }
// case NOT_RANGE -> {
// Collection<?> collection = castToCollection(convertedValue);
// Object[] valuesToCompare = collection.toArray();
// if (valuesToCompare.length != 2) {
// throw new IllegalArgumentException("NOT_RANGE operator requires exactly 2
// values");
// }
// Predicate between = buildBetweenPredicate(path, valuesToCompare[0],
// valuesToCompare[1], cb);
// yield cb.not(between);
// }
// case CUSTOM -> {
// Optional<CustomOperatorProvider> provider =
// OperatorProviderRegistry.getProvider(definition.op());
// if (provider.isPresent()) {
// yield provider.get().toResolver(definition).resolve((Root) root, query, cb);
// }
// throw new IllegalStateException("No provider found for CUSTOM operator <" +
// definition.op() + ">");
// }
// };
// };
...
//
// /**
// * Converts the value from FilterDefinition to the appropriate type based on
// the path type
// * and operator requirements (single value vs collection).
// */
// private static <P extends Enum<P> & PropertyReference> Object
// convertValueForPath(
// FilterDefinition<P> definition, Class<?> targetType, FilterConfig
// filterConfig) {
//
// Object value = definition.value();
//
...
// }
//
// // For operators that explicitly require collections (IN, NOT_IN, RANGE,
// NOT_RANGE)
// if (requiresCollection(definition.operator())) {
// return TypeConversionUtils.convertToTypedCollection(value, targetType,
// filterConfig.getEnumMatchMode());
// }
//
// // For singleton operators: if value is already a collection, keep it as
// collection
...
// return TypeConversionUtils.convertValue(targetType, value,
// filterConfig.getEnumMatchMode());
// }
//
// /**
...
//
// /**
// * Builds an OR predicate if value is a collection, otherwise returns a single
// predicate.
// * For example: EQ with [1, 2, 3] becomes (field = 1 OR field = 2 OR field =
// 3)
// */
// private static <T> Predicate buildOrPredicateIfCollection(
// Expression<T> expression,
...
//
// /**
// * Builds an AND predicate if value is a collection, otherwise returns a
// single predicate.
// * For example: NE with [1, 2, 3] becomes (field != 1 AND field != 2 AND field
// != 3)
// */
// private static <T> Predicate buildAndPredicateIfCollection(
// Expression<T> expression,
...
// */
// @SuppressWarnings("unchecked")
// private static <T extends Comparable<? super T>> Path<T>
// castToComparablePath(Path<?> path) {
// return (Path<T>) path;
// }
//
// /**
...
// */
// private static <T extends Comparable<? super T>> Comparable<?>
// castToComparable(Object value) {
// if (value instanceof Comparable<?> cmp) {
// return cmp;
// }
// throw new IllegalArgumentException("Value is not Comparable: " + value);
...
// */
// @SuppressWarnings({"rawtypes","unchecked"})
// private static Predicate buildBetweenPredicate(Path<?> path, Object start,
// Object end, CriteriaBuilder cb) {
// Path<? extends Comparable> comparablePath = castToComparablePath(path);
// Comparable startValue = castToComparable(start);
// Comparable endValue = castToComparable(end);
// return cb.between((Expression) comparablePath, startValue, endValue);
