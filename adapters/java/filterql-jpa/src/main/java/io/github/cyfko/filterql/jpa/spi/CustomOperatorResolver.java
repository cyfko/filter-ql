 ...
 * <p>
 * This functional interface provides a centralized way to handle custom
 * operators
 ...
 * <h2>Use Cases</h2>
 * <ul>
 * <li><strong>Custom operators:</strong> Implement operators like SOUNDEX,
 * LEVENSHTEIN,
 * GEO_WITHIN, FULL_TEXT that work across multiple properties</li>
 * <li><strong>Override behavior:</strong> Customize how standard operators (EQ,
 * MATCHES, etc.)
 * work for specific properties</li>
 * <li><strong>Cross-cutting concerns:</strong> Apply tenant filtering,
 * soft-delete handling,
 * or audit restrictions transparently</li>
 * <li><strong>Dynamic behavior:</strong> Change operator semantics based on
 * runtime context
 ...
 *         UserProperty.class,
 *         mappingBuilder).withCustomOperatorResolver(soundexResolver);
 * }</pre>
 *
 ...
 * access shared state. The resolver may be called concurrently from multiple
 * threads.
 * </p>
 *
 * @param <P> the property reference enum type (must implement
...
public interface CustomOperatorResolver<P extends Enum<P> & PropertyReference> {
     ...
     * @param op   the operator code (standard like "EQ", "MATCHES" or custom like
     *             "SOUNDEX")
     ...
     *                                  should not be delegated (e.g., unsupported
     *                                  property for this operator)
     */
