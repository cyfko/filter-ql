 ...
 */
public class MultiQueryFetchStrategyV2 implements ExecutionStrategy<List<Map<String, Object>>> {

    private static final Logger logger = Logger.getLogger(MultiQueryFetchStrategyV2.class.getName());
     ...
     */
    public MultiQueryFetchStrategyV2(Class<?> projectionClass) {
     ...
     *                         none needed
     */
    public MultiQueryFetchStrategyV2(Class<?> projectionClass, InstanceResolver instanceResolver) {
    ...
    public <Context> List<Map<String, Object>> execute(Context ctx, PredicateResolver<?> pr,
        ...
        if (execCtx.plan.hasComputedFields()) {
            handleComputedFields(rootResults.values(), execCtx);
        }

        // 5. Convert to final Map format (single pass at the end)
    ...
    private void handleComputedFields(Collection<RowBuffer> results, ExecutionContext ctx) {
        ...
        for (RowBuffer row : results) {
            for (ComputedFieldInfo info : computedFields) {
                // Gather dependencies using pre-computed indices
                ...
                for (int i = 0; i < params.length; i++) {
                    ...
                    if (slot >= 0) {
                        ...
                        params[i] = row.getOrNull(info.dependencyPaths()[i]);
                    }
                }
                ...
                try {
                    ...
                    for (String depPath : info.dependencyPaths()) {
                        ...
                        if (!isDirectlyProjected) {
                            int depSlot = ctx.plan.rootSchema().indexOfEntity(depPath);
                            if (depSlot >= 0) {
                                row.set(depSlot, null);
                            }
                        }
                    }
                } catch (Exception e) {
